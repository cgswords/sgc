...
;;;   into a define rather than into a define.
;;;
...
;;;
;;;   (cfunc? x) returns true iff x is a valid c function name (.i.e., 
;;;   one whose name is a alphanumeric string that doesn't start with a
;;;   number.)
;;;
...
(library (framework helpers)
  (export
    ...
    tag-procedure disp-procedure-code disp-procedure-length disp-procedure-data 
    $contmark
    ...
    emit-label emit-call emit-section
    ...
    unique-name unique-name-count extract-suffix unique-label unique-suffix
    extract-root
    ...
    rax rcx rdx rbx rbp rsp rsi rdi r8 r9 r10 r11 r12 r13 r14 r15
...
; 	 return-address-register allocation-pointer-register)
  ...
  (define registers
    '(rax rsp rcx rdx rbx rbp rsi rdi r8 r9 r10 r11 r12 r13 r14 r15))
...
(define (index->frame-var n)
  (when (> n (max-frame-var))
    (begin
      (errorf 'index->frame "index not in range of max-frame-var ~a~a~n" `(index ,n) `(max ,(max-frame-var)))
      (error 'index->frame "index not in range of max-frame-var"
      `(index ,n) `(max ,(max-frame-var)))))
  (string->symbol (format "fv~d" n)))

;;; cfuncs
;;; This implementation is incomplete and should check for proper formatting
...
(define tag-label  #b100)

;; tag for symbols is 5 if and when we get rid of stack ticks
;; tag for strings is 7
;; in a real implementation I would probably make strings an
;; special case of vectors
;; Non fixnum imediates 6
(define tag-boolean    #b00000110)
(define mask-boolean   #b11110111)
(define $contmark      #b00000101)
...
(library (framework helpers frame-variables)
  ...
  (define-frame-variables 0 1000)

  (define (prepare-frame-variables) 
    (void))
)
